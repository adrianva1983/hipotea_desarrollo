						...
						if($campoHitoExpediente->getAvisarColaborador() and $camposHitoExpedienteColaboradores){
							if($expediente->getIdComercial() != null){
								...
								if($expediente->getIdComercial()->getMailerTransport()!= null && $expediente->getIdComercial()->getMailerTransport()!= ""){
									$from = array($expediente->getIdComercial()->getMailerUser() => $expediente->getIdComercial()->getUsername()." ".$expediente->getIdComercial()->getApellidos(). ' - Grupo Negociador');
									if($expediente->getIdComercial()->getFirmaCorreo() != null && $expediente->getIdComercial()->getFirmaCorreo() != ""){
										...
										$imagenCorreo = "firma_base.jpg";
									}
									$subBody = '<img src="https://areaprivada.gruponegociador.es/uploads/'.$imagenCorreo.'"><p style="font-size:9px;color:#CCC;">Este mensaje ha sido enviado por GRUPO NEGOCIADOR, SLU, Este mensaje y sus archivos adjuntos van dirigidos exclusivamente a su destinatario, pudiendo contener información confidencial sometida a secreto profesional. No está permitida su reproducción o distribución sin la autorización expresa. Si usted no es el destinatario final por favor elimínelo e infórmenos por esta vía. De acuerdo con lo establecido por el Reglamento (UE) 2016/679, en materia de Protección de datos, le informamos que el Responsable del Tratamiento de sus datos es GRUPO NEGOCIADOR ALR, S.L. Utilizamos sus datos para prestarle los servicios que nos ha solicitado así como enviarle comunicaciones comerciales que sean de su interés. Legitimados en Ejecución de un contrato: Prestación de los servicios solicitados. Interés legítimo del Responsable: Envío de comunicaciones comerciales. No se cederán sus datos a terceros salvo obligación legal. Asimismo, le informamos que tiene derecho a acceder, rectificar y suprimir los datos, así como otros derechos, indicados en la información adicional, que puede ejercer dirigiéndose a C/ Isabel La Monja 7, 14711, Córdoba. Puede consultar información adicional y detallada sobre protección de datos dirigiéndose al correo electrónico info@gruponegociador.es o se puede enviar un archivo adjunto al mensaje.</p>';
								}else{
									$from = array($this->getParameter('mailer_user') => 'Grupo Negociador');
									$subBody = '<img src="https://areaprivada.gruponegociador.es/uploads/firma_base.jpg"><p style="font-size:9px;color:#CCC;">Este mensaje ha sido enviado por GRUPO NEGOCIADOR, SLU, Este mensaje y sus archivos adjuntos van dirigidos exclusivamente a su destinatario, pudiendo contener información confidencial sometida a secreto profesional. No está permitida su reproducción o distribución sin la autorización expresa. Si usted no es el destinatario final por favor elimínelo e infórmenos por esta vía. De acuerdo con lo establecido por el Reglamento (UE) 2016/679, en materia de Protección de datos, le informamos que el Responsable del Tratamiento de sus datos es GRUPO NEGOCIADOR ALR, S.L. Utilizamos sus datos para prestarle los servicios que nos ha solicitado así como enviarle comunicaciones comerciales que sean de su interés. Legitimados en Ejecución de un contrato: Prestación de los servicios solicitados. Interés legítimo del Responsable: Envío de comunicaciones comerciales. No se cederán sus datos a terceros salvo obligación legal. Asimismo, le informamos que tiene derecho a acceder, rectificar y suprimir los datos, así como otros derechos, indicados en la información adicional, que puede ejercer dirigiéndose a C/ Isabel La Monja 7, 14711, Córdoba. Puede consultar información adicional y detallada sobre protección de datos dirigiéndose al correo electrónico info@gruponegociador.es o se puede enviar un archivo adjunto al mensaje.</p>';
								}
								...
								if($expediente->getIdTecnico()->getMailerTransport()!= null && $expediente->getIdTecnico()->getMailerTransport()!= ""){
									$from = array($expediente->getIdTecnico()->getMailerUser() => $expediente->getIdTecnico()." ".$expediente->getIdTecnico()->getApellidos(). ' - Grupo Negociador');
							...
							$body.=$subBody;
							$mensaje = (new Swift_Message('Grupo Negociador: Nuevo documento de '.$this->getUser()->getUsername() . " " . $this->getUser()->getApellidos()))
								->setFrom($from)
					...
					if ($ficheroCampo) {
						try {
							...
							if($campoHitoExpediente->getAvisarColaborador() and $camposHitoExpedienteColaboradores){
								...
								$body.=$subBody;
								// $from = array($this->getParameter('mailer_user') => 'Grupo Negociador');
								$mensaje = (new Swift_Message('Grupo Negociador: Nuevo documento firmado de '.$this->getUser()->getUsername() . " " . $this->getUser()->getApellidos()))
									->setFrom($from)
									->setTo($camposHitoExpedienteColaboradores->getIdAgenteColaborador()->getEmail())
									->setBody($body, 'text/html');
				...
				if ($jsonRecibido['password'] === $jsonRecibido['password2']) {
					...
					if ($usuario->getPoliticaPrivacidad() && $usuario->getContratoFipre()) {
						...
						if (count($violaciones) === 0) {
							...
							$managerEntidad->flush();
							$from = array($this->getParameter('mailer_user') => 'Grupo Negociador');
							$mensaje = (new Swift_Message('Activar cuenta en Grupo Negociador.'))
								->setFrom($from)
