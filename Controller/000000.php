...
public function agregarModificarExpedienteAction(Request $request, RouterInterface $router, $id, Swift_Mailer $mailer)
{
    ...
    switch ($this->getUser()->getRoles()[0]) {
        case 'ROLE_ADMIN':
            $formularioExpedienteOpciones = array(
                ...
                'responsablesZona' => $doctrine->getRepository(UsuarioEntidad::class)->matching(Criteria::create()
                    ->where(Criteria::expr()->eq('role', 'ROLE_RESPONSABLE_ZONA'))
                    ->andWhere(Criteria::expr()->orX(
                        Criteria::expr()->gte('fechaCaducidad', new DateTime()),
                        Criteria::expr()->isNull('fechaCaducidad')
                    ))
        ...
        case 'ROLE_COLABORADOR':
            ...
            // Colaborador solo puede ver sus expedientes
            if ($expediente->getIdColaborador() !== $this->getUser()) {
                $this->addFlash('danger', 'El expediente con id="' . $id . '" es de otro colaborador.');
                return $this->redirectToRoute('lista_expedientes');
            }
        ...
        case 'ROLE_RESPONSABLE_ZONA':
            ...
            case 'ROLE_JEFE_INMOBILIARIA':
            case 'ROLE_JEFE_OFICINA':
                // Jefes de inmobiliaria y oficina: pueden editar expedientes de su inmobiliaria
                $formularioExpedienteOpciones = array(
                    'esAdmin' => true,
                    'colaboradores' => $doctrine->getRepository(UsuarioEntidad::class)->matching(Criteria::create()
                        ->where(Criteria::expr()->eq('role', 'ROLE_COLABORADOR'))
    ...
    if ($request->isMethod('POST') && $formulariosValidos) {
     ...
     Esta comunicación es privada y los documentos adjuntos a la misma son confidenciales y dirigidos exclusivamente a los destinatarios de los mismos, por lo que su divulgación está expresamente prohibida. Por favor, si Ud. no es uno de dichos destinatarios, sírvase notificarnos este hecho y no copie o revele su contenido a terceros por ningún medio.</p>';
        ...
        // AHORA CREAMOS LOS CAMPOS DE LA FASE DE RECOGIDA DE DOCUMENTOS SI ES EXPEDIENTE NUEVO PARA COLABORADOR Y CLIENTE
        if(!isset($id) && ($this->getUser()->getRoles()[0] == 'ROLE_COLABORADOR' 
            || $this->getUser()->getRoles()[0] == 'ROLE_CLIENTE'
            || $this->getUser()->getRoles()[0] == 'ROLE_JEFE_INMOBILIARIA'
            || $this->getUser()->getRoles()[0] == 'ROLE_JEFE_OFICINA'
            || $this->getUser()->getRoles()[0] == 'ROLE_RESPONSABLE_ZONA')){
            $fases = $doctrine->getRepository(FaseEntidad::class)->findBy(array('tipo' => 1), array(
                'orden' => 'ASC'
            ));
        ...
        if ($formularioExpediente->isSubmitted() && $formularioExpediente->isValid()) {
            if ($expediente->getIdComercial() == null){
                // Comprobamos si tiene colaborador asignado
                if ($expediente->getIdColaborador()){
                    $comercial = $expediente->getIdColaborador()->getIdInmobiliaria()->getIdComercial();
        ...
        if (($this->getUser()->getRoles()[0] === 'ROLE_CLIENTE' 
            || $this->getUser()->getRoles()[0] === 'ROLE_COLABORADOR'
            || $this->getUser()->getRoles()[0] === 'ROLE_JEFE_INMOBILIARIA'
            || $this->getUser()->getRoles()[0] === 'ROLE_JEFE_OFICINA'
            || $this->getUser()->getRoles()[0] === 'ROLE_RESPONSABLE_ZONA') && !$irADocumentos) {
            return $this->redirectToRoute('modificar_expediente', array(
                'id' => $expediente->getIdExpediente()
            ));//Recarga forzada para poder ver la imagen en el plugin Dropify
        }elseif (($this->getUser()->getRoles()[0] === 'ROLE_CLIENTE' 
            || $this->getUser()->getRoles()[0] === 'ROLE_COLABORADOR'
            || $this->getUser()->getRoles()[0] === 'ROLE_JEFE_INMOBILIARIA'
            || $this->getUser()->getRoles()[0] === 'ROLE_JEFE_OFICINA'
            || $this->getUser()->getRoles()[0] === 'ROLE_RESPONSABLE_ZONA') && $irADocumentos) {
    ...
    // dump($variablesTwig['grupos_hito_expediente']);
    // die;
    // Obtener valor del campo 707 (Activar Blender) para clientes
    $valor_campo_707 = null;
    try {
        // Búsqueda simple y directa sin loops innecesarios
        $camposExpediente = $doctrine->getRepository(CampoHitoExpedienteEntidad::class)->findBy([
            'idExpediente' => $expediente,
            'idCampoHito' => 707
        ]);
        ...
        if (!empty($camposExpediente)) {
            $campoEncontrado = $camposExpediente[0];
            $opcion = $campoEncontrado->getIdOpcionesCampo();
            if ($opcion) {
                // Intentar diferentes nombres de método
                if (method_exists($opcion, 'getOpcion')) {
                    $valor_campo_707 = $opcion->getOpcion();
                } elseif (method_exists($opcion, 'getValor')) {
                    $valor_campo_707 = $opcion->getValor();
                } elseif (method_exists($opcion, 'getNombre')) {
                    $valor_campo_707 = $opcion->getNombre();
                } elseif (method_exists($opcion, 'getTexto')) {
                    $valor_campo_707 = $opcion->getTexto();
                }
            }
        }
    } catch (\Exception $e) {
